            ...
            try {
                ...
                if (file.exists) {
                    ...
                    if (opened) {
                        ...
                        while (!file.eof) {
                            $.writeln("Reading a line from the file");
                            var line = file.readln();
                            $.writeln("Read line: " + line);
                            var parts = line.split(": ");
                            $.writeln("Split line into parts: " + parts.length);

                            if (parts.length === 2) {
                                $.writeln("parts[0] type: " + typeof parts[0]);
                                $.writeln("parts[1] type: " + typeof parts[1]);
                                var country = trimString(parts[0]);
                                var translation = trimString(parts[1]);
                                translations[country] = translation;
                                $.writeln("Parsed translation: " + country + " => " + translation);
                            } else {
                                $.writeln("Line did not split correctly: " + line);
                            }
                        }
                        file.close();
        ...
        selectFolderBtn.onClick = function() {
            ...
            if (selectedFolder) {
                ...
                if (audioFolder.exists && picturesFolder.exists) {
                    ...
                    var translations = loadTranslations("C:/Users/aldan/AppData/Local/Programs/Python/Python312/saves/translations.txt");
